    ...
    BOOL swizzleSuccess =
        [swizzler swizzleClass:self
            replaceInstanceMethod:@selector(runTransitionForCurrentStateAnimated:handoffData:)
                       withMethod:@selector
                       (greyswizzled_runTransitionForCurrentStateAnimated:handoffData:)];
    GREYFatalAssertWithMessage(
        swizzleSuccess, @"Cannot swizzle UIPresentationController "
...
- (int)greyswizzled_runTransitionForCurrentStateAnimated:(int)state handoffData:(id)data {
  ...
  objc_setAssociatedObject(self, @selector(greyswizzled_transitionDidFinish:),
                           object, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
  return INVOKE_ORIGINAL_IMP2(
      int,
      @selector(greyswizzled_runTransitionForCurrentStateAnimated:handoffData:),
      state, data);
}

- (int)greyswizzled_transitionDidFinish:(int)state {
  GREYAppStateTrackerObject* object = objc_getAssociatedObject(
      self, @selector(greyswizzled_transitionDidFinish:));
  UNTRACK_STATE_FOR_OBJECT(kGREYPendingGestureRecognition, object);
  objc_setAssociatedObject(self, @selector(greyswizzled_transitionDidFinish:),
                           nil, OBJC_ASSOCIATION_ASSIGN);
  return INVOKE_ORIGINAL_IMP1(int, @selector(greyswizzled_transitionDidFinish:),
                              state);
}
