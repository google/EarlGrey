...
@implementation UIWebView (GREYApp)
...
- (void)grey_clearPendingInteraction {
  ...
  objc_setAssociatedObject(self, @selector(grey_pendingInteractionForTime:), nil,
...
- (void)grey_pendingInteractionForTime:(NSTimeInterval)seconds {
  ...
  id<GREYIdlingResource> timedResource = [GREYTimedIdlingResource resourceForObject:self
                                                              thatIsBusyForDuration:seconds
                                                                               name:resourceName];
  objc_setAssociatedObject(self, @selector(grey_pendingInteractionForTime:), timedResource,
...
- (void)grey_trackAJAXLoading {
  ...
  objc_setAssociatedObject(self, @selector(grey_trackAJAXLoading), object,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)grey_untrackAJAXLoading {
  ...
  objc_setAssociatedObject(self, @selector(grey_trackAJAXLoading), nil, OBJC_ASSOCIATION_ASSIGN);
}

- (void)grey_setIsLoadingFrame:(BOOL)loading {
  objc_setAssociatedObject(self, kUIWebViewLoadingStateKey, @(loading),
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (id<UIWebViewDelegate>)grey_proxyDelegateFromDelegate:(id<UIWebViewDelegate>)delegate {
  ...
  if (![proxyDelegate isKindOfClass:[GREYUIWebViewDelegate class]]) {
    ...
    objc_setAssociatedObject(self, @selector(greyswizzled_delegate), delegateList,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
  }
  return proxyDelegate;
}
