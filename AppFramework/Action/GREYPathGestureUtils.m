 ...
 */
static CGFloat EdgePanDetectionLength(void) {
  static CGFloat detectionLength = NAN;
  ...
  // UIScreenEdgePanGestureRecognizer and scrolling are not supported on tvOS.
    ...
    if ([popGestureRecognizer respondsToSelector:@selector(_edgeRegionSize)]) {
      detectionLength = [popGestureRecognizer _edgeRegionSize];
    }
#endif  // TARGET_OS_IOS
  });
...
+ (NSArray *)touchPathForGestureInView:(UIView *)view
  ...
  UIEdgeInsets edgeInset =
      UIEdgeInsetsMake(0.0f, EdgePanDetectionLength(), 0.0f, EdgePanDetectionLength());
