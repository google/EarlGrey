 ...
 */
static NSString *const kGREYAnalyticsTrackingID = @"UA-54227235-6";
...
@implementation GREYAnalytics {
   ...
   */
  BOOL _earlgreyWasCalledWithinXCTestContext;
}
...
- (void)didInvokeEarlGrey {
  ...
  if (!_earlgreyWasCalledWithinXCTestContext && [XCTestCase grey_currentTestCase]) {
    _earlgreyWasCalledWithinXCTestContext = YES;
  }
}
...
+ (void)sendEventHitWithTrackingID:(NSString *)trackingID
  ...
  NSMutableString *payload =
      [[NSMutableString alloc] initWithFormat:@"v=1"
                                              @"&t=event"
                                              @"&tid=%@"
                                              @"&cid=%@"
                                              @"&ec=%@"
...
- (void)grey_testCaseInstanceDidTearDown {
  if (_earlgreyWasCalledWithinXCTestContext) {
    ...
    if (GREY_CONFIG_BOOL(kGREYConfigKeyAnalyticsEnabled)) {
      NSString *bundleIDMD5 = [[[XCUIApplication alloc] init].bundleID grey_md5String];
      if (!bundleIDMD5) {
      ...
      NSString *testCaseName = [testCase grey_testClassName];
      NSString *testMethodName = [testCase grey_testMethodName];
      NSString *testCaseMD5 =
          [[NSString stringWithFormat:@"%@::%@", testCaseName, testMethodName] grey_md5String];
