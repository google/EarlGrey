...
@interface GREYObjectFormatterTest : XCTestCase
@end
...
- (void)testFormatNullArray {
  ...
  XCTAssertTrue([formatted isEqualToString:@""], @"Error formatting null array");
}

- (void)testFormatEmptyArray {
  NSArray *testArray = @[];
  NSString *formatted =
      [GREYObjectFormatter formatArray:testArray prefix:@"" indent:0 keyOrder:nil];
...
- (void)testFormatArrayPrefix {
  ...
  formatted = [GREYObjectFormatter formatArray:testArray prefix:@"\t" indent:0 keyOrder:nil];
  XCTAssertTrue([formatted isEqualToString:@"\t[\n\t\"item1\",\n\t\"item2\",\n\t\"item3\"\n\t]"],
                @"Error formatting array with prefix");
}
...
- (void)testFormatArrayOfArray {
  NSArray *testArray =
      @[ @[ @"item11", @"item12" ], @[ @[ @"item211", @"item212" ], @[ @"item221" ] ] ];
  NSString *formatted = [GREYObjectFormatter formatArray:testArray
                                                  prefix:@"  "
                                                  indent:kGREYObjectFormatIndent
                                                keyOrder:nil];
  NSString *expected =
      @"  [\n    [\n      \"item11\",\n      \"item12\"\n    ],"
       "\n    [\n      [\n        \"item211\",\n        \"item212\"\n      ],"
       "\n      [\n        \"item221\"\n      ]\n    ]\n  ]";
  XCTAssertTrue([formatted isEqualToString:expected], @"Error formatting array of array");
}

- (void)testFormatArrayOfArrayWithEmpty {
  NSArray *testArray = @[ @[ @"item11", @"item12" ], @[ @[ @"item211", @"item212" ], @[] ] ];
  NSString *formatted = [GREYObjectFormatter formatArray:testArray
                                                  prefix:@"  "
                                                  indent:kGREYObjectFormatIndent
                                                keyOrder:nil];
...
- (void)testFormatArrayUnsupportedError {
  ...
  XCTAssertNil(formatted, @"Error getting unsupported error for dictionary formatting");
}

- (void)testFormatNullDictionary {
  NSString *formatted =
      [GREYObjectFormatter formatDictionary:nil prefix:nil indent:0 hideEmpty:NO keyOrder:nil];

  XCTAssertTrue([formatted isEqualToString:@""], @"Error formatting null dictionary");
}

- (void)testFormatEmptyDictionary {
  NSDictionary *testDict = @{};
  NSString *formatted =
      [GREYObjectFormatter formatDictionary:testDict prefix:nil indent:0 hideEmpty:NO keyOrder:nil];
...
- (void)testFormatStringDictionary {
  ...
  NSDictionary *parsed = [NSJSONSerialization JSONObjectWithData:jsonData
                                                         options:NSJSONReadingMutableContainers
                                                           error:nil];
  XCTAssertTrue([parsed[@"key1"] isEqualToString:@"value1"], @"Error formatting string dictionary");
  XCTAssertTrue([parsed[@"key2"] isEqualToString:@"value2"], @"Error formatting string dictionary");
  XCTAssertTrue([parsed[@"key3"] isEqualToString:@"value3"], @"Error formatting string dictionary");
  XCTAssertNil(parsed[@"key4"], @"Error formatting string dictionary");
}
...
- (void)testFormatStringDictionaryWithEmptyHidden {
  ...
  NSString *formatted = [GREYObjectFormatter formatDictionary:testDict
                                                    ...
                                                    hideEmpty:YES
                                                     keyOrder:@[ @"key1", @"key2", @"key3" ]];
  NSString *expected = @"{\n\"key1\" : \"value1\",\n\"key3\" : \"value3\"\n}";
  XCTAssertTrue([formatted isEqualToString:expected], @"Error formatting with empty key hidden");
}
...
- (void)testFormatStringDictionaryPrefix {
  ...
  NSString *formatted = [GREYObjectFormatter formatDictionary:testDict
                                                    ...
                                                    hideEmpty:YES
                                                     keyOrder:@[ @"key1", @"key2", @"key3" ]];
  NSString *expected = @"  {\n  \"key1\" : \"value1\",\n  \"key2\" : \"value2\"\n  }";
  XCTAssertTrue([formatted isEqualToString:expected], @"Error formatting prefix dictionary");
}
...
- (void)testFormatStringDictionaryIndent {
  ...
  NSString *formatted = [GREYObjectFormatter formatDictionary:testDict
                                                    ...
                                                    hideEmpty:YES
                                                     keyOrder:@[ @"key1", @"key2", @"key3" ]];
  NSString *expected = @"{\n  \"key1\" : \"value1\",\n  \"key2\" : \"value2\"\n}";
  XCTAssertTrue([formatted isEqualToString:expected], @"Error formatting indent dictionary");
}
...
- (void)testFormatStringDictionaryOfDictionary {
  NSDictionary *testDict = @{
    ...
    @"key3" : @{@"key31" : @{@"key311" : @"value311", @"key312" : @"value312"}}
  };
  NSString *formatted = [GREYObjectFormatter formatDictionary:testDict
                                                       prefix:@"  "
                                                       indent:kGREYObjectFormatIndent
...
- (void)testFormatMixedDictionary {
  NSDictionary *testDict = @{
    ...
    @"key3" : @{
      @"key31" : @{@"key311" : @"value311", @"key312" : @"value312"},
  ...
  NSDictionary *parsed = [NSJSONSerialization JSONObjectWithData:jsonData
                                                         options:NSJSONReadingMutableContainers
                                                           error:nil];
  XCTAssertTrue([parsed[@"key1"] isEqualToString:@"value1"], @"Error formatting mixed dictionary");
  XCTAssertTrue([parsed[@"key2"][@"key21"] isEqualToString:@"value21"],
                @"Error formatting mixed dictionary");
  XCTAssertTrue([parsed[@"key2"][@"key23"][0] count] == 0, @"Error formatting mixed dictionary");
  ...
  XCTAssertTrue([parsed[@"key5"][2] compare:@(53)] == NSOrderedSame,
                @"Error formatting mixed dictionary");
  XCTAssertNil(parsed[@"key6"], @"Error formatting mixed dictionary");
  XCTAssertTrue([parsed[@"key7"] count] == 0, @"Error formatting mixed dictionary");
}
