...
- (void)testLappingWhenWatchIsStartedAndStopped {
  ...
  XCTAssertThrows([stopwatch lapAndReturnTime],
                  @"Stopwatch lapping cannot be done when the "
...
- (void)testCheckingElapsedTimeWhenWatchIsStartedAndStopped {
  ...
  XCTAssertNotEqual(noStopInterval, interval,
                    @"Should be able to get elapsed time without "
                    @"calling stop");
  [stopwatch stop];
  XCTAssertGreaterThan([stopwatch elapsedTime], noStopInterval,
                       @"Since the stopwatch was stopped "
...
- (void)testStopwatchTimeOnDifferentActionsBeingPerformedBetweenChecks {
  ...
  XCTAssertNotEqual(intervalOnSomeActionPerformed, intervalOnNoActionPerformed,
...
- (void)testStopwatchWithinStopwatch {
  ...
  XCTAssertGreaterThan(timeForOuterStopwatch, timeForInnerStopwatch,
...
- (void)testStopwatchTimesWithASleepBetweenThem {
  ...
  CFRunLoopRunInMode(kCFRunLoopDefaultMode, 0.01, FALSE);
  [testStopwatch stop];
  NSTimeInterval difference = [testStopwatch elapsedTime] - [benchmarkStopwatch elapsedTime];
  XCTAssertGreaterThan(difference, 0.01f,
                       @"The sleep should provide at least a difference of 0.1 "
                       @"second.");
}
...
- (void)testStopwatchStoppingWithoutStarting {
  ...
  XCTAssertThrows([stopwatch elapsedTime],
                  @"Calling elapsed time stop on a stopwatch that"
...
- (void)testStopwatchWithLappingAndAddingStartAndStopElapsedTimes {
  ...
  XCTAssertGreaterThan(secondElapsedTime, elapsedTime,
                       @"Progressive elapsed times checks must be "
  ...
  XCTAssertGreaterThan(thirdElapsedTime, secondElapsedTime,
                       @"Progressive elapsed times checks "
