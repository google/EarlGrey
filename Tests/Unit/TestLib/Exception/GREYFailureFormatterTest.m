...
@interface GREYFailureFormatterTest : XCTestCase
@end
...
- (void)testFormatFailureWithError {
  ...
  GREYError *error =
      GREYErrorMake(@"com.google.earlgrey.ElementInteractionErrorDomain", 4, errorDescription);
  ...
  XCTAssertTrue([failureLines[0] isEqualToString:expected], @"Error format failure with error.");
  expected = [NSString stringWithFormat:@"%@: %@", @"File", filePath];
  XCTAssertTrue([failureLines[2] isEqualToString:expected], @"Error format failure with error.");
  expected = [NSString stringWithFormat:@"%@: %@", @"Function", functionName];
  XCTAssertTrue([failureLines[6] isEqualToString:expected], @"Error format failure with error.");
  expected = [NSString stringWithFormat:@"%@", @"Test Failure String"];
  XCTAssertTrue([failureLines[8] isEqualToString:expected], @"Error format failure with error.");
  expected =
      [NSString stringWithFormat:@"%@: %@", @"Bundle ID", [[NSBundle mainBundle] bundleIdentifier]];
  XCTAssertTrue([failureLines[9] isEqualToString:expected], @"Error format failure with error.");
  expected = [NSString stringWithFormat:@"%@: %@", @"Stack Trace", @"("];
  XCTAssertTrue([failureLines[11] isEqualToString:expected], @"Error format failure with error.");
  NSUInteger index;
  index = 12;
  ...
  XCTAssertLessThan(index, [failureLines count], @"Error format failure with error.");

  index += 2;
  expected = [NSString stringWithFormat:@"%@: ", @"Screenshots"];
  XCTAssertTrue([failureLines[index] hasPrefix:expected], @"Error format failure with error.");

  index += 2;
  expected = [NSString stringWithFormat:@"%@(", @"UI Hierarchy "];
  XCTAssertTrue([failureLines[index] hasPrefix:expected], @"Error format failure with error.");
}

- (void)testUIHierarchyCollection {
  NSString *hierarchy = @"FooTestHierarchy";
  NSString *formattedFailureWithUIHierarchy =
      [GREYFailureFormatter formatFailureForTestCase:self
                                        failureLabel:@""
                                         failureName:@""
                                            filePath:@""
                                          lineNumber:1
                                      ...
                                      appScreenshots:@{}
                                           hierarchy:hierarchy
                                              format:@"Foo:"];
  XCTAssertTrue([formattedFailureWithUIHierarchy containsString:hierarchy]);
}
