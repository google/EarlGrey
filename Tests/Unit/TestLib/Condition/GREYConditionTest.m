...
@interface GREYConditionTest : XCTestCase
@end
...
- (void)testConditionMet {
  ...
  GREYCondition *condition = [GREYCondition conditionWithName:@"test"
                                                        block:^BOOL() {
                                                          conditionCalled = YES;
                                                          return YES;
                                                        }];
  BOOL res __attribute__((unused)) = [condition waitWithTimeout:0.1];
...
- (void)testConditionMetAfterMultipleAttempts {
  ...
  XCTAssertTrue(conditionCalled, @"Condition should have been called.");
  XCTAssertEqual(numExec, 3,
                 @"Condition should have met the third time and returned immediately. "
                 @"Current condition execution count: %d",
                 numExec);
}
...
- (void)testDrainUntilIdleInsideCondition {
  ...
  GREYCondition *condition =
      [GREYCondition conditionWithName:@"test"
                                 block:^{
