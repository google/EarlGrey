...
- (void)testIsNotVisibleReturnsYesForNilElements {
  XCTAssertTrue([GREYVisibilityChecker isNotVisible:nil], @"Nil elements should not be visible");
}
...
- (void)testAccessibilityElementWithNoContainerViewIsNotVisible {
  ...
  XCTAssertEqual(percentVisible, 0,
...
- (void)testVisibilityOfViewWithBeforeAndShiftedAfterImage {
  ...
  // Before Image.
  [self addToScreenshotListReturnedByScreenshotter:[self grey_imageOfSize:size
...
- (void)testVisibilityOfViewWithAlphaAndBeforeAndShiftedAfterImage {
  ...
  XCTAssertEqual([GREYVisibilityChecker percentVisibleAreaOfElement:view], 1.0f,
                 ...
                 @" by 128");
  XCTAssertEqual((float)view.alpha, 0.6f, @"Visibility checker must not change alpha values");
  XCTAssertEqual((float)window.alpha, 0.7f, @"Visibility checker must not change alpha values");
}
...
- (void)testVisibilityOfViewWithSameImageReturnedForBeforeAndAfter {
  ...
  [self addToScreenshotListReturnedByScreenshotter:image];
  [self addToScreenshotListReturnedByScreenshotter:image];
  [self addToScreenshotListReturnedByScreenshotter:image];
  [self addToScreenshotListReturnedByScreenshotter:image];
  [self addToScreenshotListReturnedByScreenshotter:image];
...
// TODO: Move testVisibilityOfElementWithSameImageReturnedForContainerViewBeforeAndAfter // NOLINT
// to the App Lib to utilize the Sufficiently Visible Matcher from GREYMatchers.
...
- (void)testVisibileForInteractionIsYesForOutOfBoundsActivationPointButVisiblePortion {
  ...
  XCTAssertFalse(
      CGPointEqualToPoint([view accessibilityActivationPoint],
...
- (void)testVisibileForInteractionIsYesForOutOfScreenBoundsActivationPoint {
  ...
  CGRect frameRect = CGRectMake(-size.width / 2 - 1, -size.height / 2 - 1, size.width, size.height);
...
- (void)testVisibleForInteractionIsNoForHiddenActivationPointAndCenterOfVisibleArea {
  ...
  // For each invocation of isVisibleForInteraction, 2 screenshots are needed, for before and after.
  [self addToScreenshotListReturnedByScreenshotter:before];
  [self addToScreenshotListReturnedByScreenshotter:after];
...
- (void)testVisibleForInteractionIsNoForLowVisiblePixelCount {
  ...
  CGSize imageSizeInPixels =
      CGSizeMake(before.size.width * before.scale, before.size.height * before.scale);

  GREYVisibilityDiffBuffer diffBuffer = GREYVisibilityDiffBufferCreate(
      (size_t)imageSizeInPixels.width, (size_t)imageSizeInPixels.height);
  GREYVisiblePixelData visiblePixels = [GREYVisibilityChecker grey_countPixelsInImage:before.CGImage
...
- (void)testIsNotVisibleIsUsingCache {
  ...
  // Before screenshot.
  [self addToScreenshotListReturnedByScreenshotter:[self grey_imageOfSize:imageSize
                                                                withColor:[UIColor whiteColor]]];
  // After screenshot.
  [self addToScreenshotListReturnedByScreenshotter:[self grey_imageOfSize:imageSize
                                                                withColor:[UIColor blackColor]]];
 ...
 */
- (CGRect)grey_visibleRectForSize:(CGSize)size area:(CGRect)targetArea areaHidden:(BOOL)hidden {
...
- (void)testVisibleAreaCorrectTopLeftVisible {
  ...
  CGRect rect = [self grey_visibleRectForSize:CGSizeMake(100, 10000) area:correct areaHidden:NO];
  XCTAssertTrue(CGRectEqualToRect(rect, correct),
                @"Visible rect returned incorrect (expected %@ got %@).",
                NSStringFromCGRect(correct), NSStringFromCGRect(rect));
}
...
- (void)testVisibleAreaCorrectCenterVisible {
  ...
  CGRect rect = [self grey_visibleRectForSize:CGSizeMake(10, 10) area:correct areaHidden:NO];
  XCTAssertTrue(CGRectEqualToRect(rect, correct),
                @"Visible rect returned incorrect (expected %@ got %@).",
                NSStringFromCGRect(correct), NSStringFromCGRect(rect));
}
...
- (void)testRectEnclosingNonExistingElementIsEmpty {
  ...
  CGRect enclosingRect = [GREYVisibilityChecker rectEnclosingVisibleAreaOfElement:view];
  XCTAssertEqualObjects([NSValue valueWithCGRect:enclosingRect],
                        [NSValue valueWithCGRect:CGRectZero], @"Rect for View is not CGRectZero.");
}
