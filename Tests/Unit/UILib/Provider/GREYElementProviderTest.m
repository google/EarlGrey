...
- (void)testTableViewCellsIncludedOnlyOnce {
  ...
  for (UIView *view in actual) {
    if (view == _cellA1) {
      XCTAssertFalse(foundCell, @"Looks like there is a duplicate cell: %@ in provider list: %@",
                     view, actual);
      foundCell = YES;
    } else if (view == cellSubview) {
      XCTAssertFalse(foundCellSubview,
                     @"Looks like there is a duplicate cell subview: %@ in provider list: %@", view,
                     actual);
...
- (void)testFullHierarchyShown {
  ...
  GREYElementProvider *provider = [GREYElementProvider providerWithRootProvider:windowProvider];
  XCTAssertEqualObjects(expected, [[provider dataEnumerator] allObjects],
...
- (void)testViewsWithViews {
  ...
  XCTAssertEqualObjects(expectedAfterNext, [dataEnumerator allObjects],
