...
@interface CGGeometry_GREYUITest : XCTestCase
@end

@implementation CGGeometry_GREYUITest
...
- (void)testCGRectIntegralInside {
  CGRect expected[][2] = {
      ...
      {CGRectMake((CGFloat)5.99, (CGFloat)6.99, (CGFloat).49, (CGFloat).49),
       CGRectMake(6, 7, 0, 0)},
  ...
  for (int i = 0; i < testSize; ++i) {
    XCTAssertTrue(CGRectEqualToRect(CGRectIntegralInside(expected[i][0]), expected[i][1]),
                  ...
                  NSStringFromCGRect(expected[i][0]), NSStringFromCGRect(expected[i][1]),
