...
#pragma mark - GREYManagedObjectContextIdlingResource Category

@interface GREYManagedObjectContextIdlingResource (GREYManagedObjectContextIdlingResourceTest)
- (dispatch_queue_t)managedObjectContextDispatchQueue;
@end

@interface GREYManagedObjectContextIdlingResourceTest : GREYAppBaseTest
@end
...
- (void)testIdleAfterInitializingAndDrainingOnBackgroundQueue {
  ...
  GREYManagedObjectContextIdlingResource *managedObjectContextIdlingResource =
      [self setUpContextIdlingResourceWithContext:managedObjectContext trackingPendingChanges:YES];
...
- (void)testBusyAfterAddingAsyncBlockOnBackgroundQueue {
  ...
  [managedObjectContext performBlock:^{
    dispatch_time_t timeout =
        dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kSemaphoreTimeoutSeconds * NSEC_PER_SEC));
...
- (void)testIdleAfterSyncMutationBlockOnBackgroundQueueWhenNotTrackingPendingChanges {
  NSManagedObjectContext *managedObjectContext =
      [self setUpContextWithConcurrencyType:NSPrivateQueueConcurrencyType];
  GREYManagedObjectContextIdlingResource *managedObjectContextIdlingResource =
      [self setUpContextIdlingResourceWithContext:managedObjectContext trackingPendingChanges:NO];
  [self drainDispatchQueue:[managedObjectContextIdlingResource managedObjectContextDispatchQueue]];
  XCTAssertTrue([managedObjectContextIdlingResource isIdleNow]);
...
- (void)testIdlingResourceWeaklyHoldsContextAndDeregistersItself {
  ...
  @autoreleasepool {
    __autoreleasing NSManagedObjectContext *managedObjectContext =
        [self setUpContextWithConcurrencyType:NSPrivateQueueConcurrencyType];
    contextIdlingResource = [self setUpContextIdlingResourceWithContext:managedObjectContext
...
- (NSManagedObjectContext *)setUpContextWithConcurrencyType:
