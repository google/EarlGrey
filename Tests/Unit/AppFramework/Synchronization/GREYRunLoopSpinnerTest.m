...
@interface GREYRunLoopSpinnerTest : GREYAppBaseTest
@end
...
- (void)testConditionMetCallbackWhenConditionTrueOnBackground {
  XCTestExpectation *expectsConditionMet = [self expectationWithDescription:@"Met the condition."];
  XCTestExpectation *expectsFinish = [self expectationWithDescription:@"Finished the wait."];
  ...
  dispatch_async(backgroundQueue, ^{
    ...
    spinner.conditionMetHandler = ^{
      XCTAssertTrue([NSThread isMainThread],
                    @"The conditionMet block should be executed on the main thread.");
      [expectsConditionMet fulfill];
    };

    BOOL result = [spinner spinWithStopConditionBlock:^BOOL {
      XCTAssertTrue([NSThread isMainThread],
                    @"The condition block should be executed on the main thread.");
      return YES;
    }];
    XCTAssertTrue(result, @"Spin result should be YES. The condition was met.");
    [expectsFinish fulfill];
  });
...
- (void)testConditionMetCallbackWhenConditionEventuallyTrue {
  ...
  XCTAssertTrue(result, @"Spin result should be YES. The condition was met eventually.");
  XCTAssertTrue(conditionMetHandlerCalled,
                @"Should call condition met handler if condition was met.");
  XCTAssertEqualObjects(conditionMetHandlerMode, kSpinnerTestMode,
                        @"Condition met handler should have been invoked on the active mode.");
...
- (void)testConditionMetCallbackWhenConditionEventuallyTrueOnBackground {
  // Execute it in the low priority background queue many times in order to produce the race
  // condition.
  const NSUInteger runTimes = 20;
  dispatch_queue_t backgroundQueue =
      dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0);
  for (NSUInteger i = 0; i < runTimes; ++i) {
    XCTestExpectation *expectsConditionMet =
        [self expectationWithDescription:@"Met the condition."];
    XCTestExpectation *expectsFinish = [self expectationWithDescription:@"Finished the wait."];
    ...
    dispatch_async(backgroundQueue, ^{
      ...
      const int conditionCheckCountLimit = 20;
      __block int currentConditionCheckCount = 0;
...
- (void)testConditionMetCallbackWhenConditionFalseOnBackground {
  XCTestExpectation *expectsFinish = [self expectationWithDescription:@"Finished the wait."];
  const CFTimeInterval spinInterval = 0.2;

  dispatch_queue_t backgroundQueue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
  dispatch_async(backgroundQueue, ^{
    GREYRunLoopSpinner *spinner = [[GREYRunLoopSpinner alloc] init];
    spinner.timeout = spinInterval;
    spinner.conditionMetHandler = ^{
      XCTFail(@"Should not call condition met handler if condition was never met.");
    };
    ...
    XCTAssertFalse(result, @"Spin result should be NO. The condition was never met.");
    XCTAssertGreaterThan(CACurrentMediaTime() + spinInterval, current);
    [expectsFinish fulfill];
  });

  // Spare one more second of timeout tolerance.
  [self waitForExpectationsWithTimeout:(1 + spinInterval) handler:nil];
}

- (void)testConditionMetWithZeroTimeout {
  XCTestExpectation *expectsFinish = [self expectationWithDescription:@"Finished the wait."];
  XCTestExpectation *expectsConditionMet = [self expectationWithDescription:@"Met the condition."];
  XCTestExpectation *expectsStopBlock = [self expectationWithDescription:@"Executed the block."];
...
- (void)testConditionMetWithZeroTimeoutOnBackgroud {
  XCTestExpectation *expectsFinish = [self expectationWithDescription:@"Finished the wait."];
  XCTestExpectation *expectsConditionMet = [self expectationWithDescription:@"Met the condition."];
  XCTestExpectation *expectsStopBlock = [self expectationWithDescription:@"Executed the block."];
...
- (void)testConditionNotMetWithZeroTimeout {
  XCTestExpectation *expectsFinish = [self expectationWithDescription:@"Finished the wait."];
  XCTestExpectation *expectsStopBlock = [self expectationWithDescription:@"Executed the block."];
...
- (void)testConditionNotMetWithZeroTimeoutOnBackground {
  XCTestExpectation *expectsFinish = [self expectationWithDescription:@"Finished the wait."];
  XCTestExpectation *expectsStopBlock = [self expectationWithDescription:@"Executed the block."];
...
- (void)testZeroDrainsOnBackground {
  XCTestExpectation *expectsFinish = [self expectationWithDescription:@"Finished the wait."];
  XCTestExpectation *expectsStopBlock = [self expectationWithDescription:@"Executed the block."];
  ...
  dispatch_async(backgroundQueue, ^{
    ...
    XCTAssertTrue(result, @"Spin result should be YES. The condition was eventually met.");
    [expectsFinish fulfill];
  });
