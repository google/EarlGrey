...
/**
 *  BOOL specifying if the state tracker is idle.
 */
static BOOL gAppStateTrackerIdle;

/**
 *  Integer specifying the timeout for any executor sync calls.
 */
static const NSUInteger kTestDrainTimeout = 1;
...
#pragma mark - GREYUIThreadExecutorTest

@interface GREYUIThreadExecutorTest : GREYAppBaseTest
@end
...
- (void)testBusyToIdleBackgroundThreadResource {
  ...
  NSError *error;
  BOOL success = [_threadExecutor executeSyncWithTimeout:kTestDrainTimeout
                                                   block:^{
                                                   }
                                                   error:&error];
...
- (void)testErrorMessageTimeoutWithBusyBackgroundResource {
  ...
  XCTAssertTrue(errorMatched, @"Reason '%@' does not contain substring '%@'", error.description,
                errorSubstring);
}
...
- (void)testTimeoutWithDrainUntilIdleWithTimeout {
  ...
  XCTAssertFalse(success,
                 @"Should timeout because timed idling resource is tracked for "
...
- (void)testDrainUntilIdleWithTimeout {
  ...
  XCTAssertTrue(success,
                @"Draining should succeed since the timed idling resource takes half the "
...
- (void)testCallSingletonTwice {
  XCTAssertEqualObjects([GREYUIThreadExecutor sharedInstance],
                        [GREYUIThreadExecutor sharedInstance], @"Not a true singleton");
}

- (void)testCallBlock {
  __block BOOL blockExecuted = NO;
  [_threadExecutor executeSyncWithTimeout:kTestDrainTimeout
                                    block:^{
                                      blockExecuted = YES;
                                    }
                                    error:nil];
...
- (void)testCallBlockFromOtherSources {
  ...
  [_threadExecutor
      executeSyncWithTimeout:kTestDrainTimeout
                       block:^{
                         XCTAssertTrue(performBlockExecuted,
                                       @"executeSync should drain perform block source above and "
...
- (void)testCallNestedBlocks {
  ...
  GREYExecBlock innerBlock1 = ^{
    innerBlock1Executed = YES;
  };
  GREYExecBlock innerBlock2 = ^{
...
- (void)testCallNestedBlocksWithRunLoopModeSwitch {
  ...
  [_threadExecutor
      executeSyncWithTimeout:kTestDrainTimeout
                       block:^{
                         XCTAssertEqualObjects([[NSRunLoop currentRunLoop] currentMode],
  ...
  [_threadExecutor executeSyncWithTimeout:kTestDrainTimeout
                                    block:^{
                                      XCTAssertEqualObjects(
                                          [[NSRunLoop currentRunLoop] currentMode], fooRunLoopMode);
