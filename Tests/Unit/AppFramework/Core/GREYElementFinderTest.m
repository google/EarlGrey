...
@interface GREYElementFinderTest : GREYAppBaseTest
@end
...
- (void)setUp {
  ...
  niceMatcher = [GREYElementMatcherBlock
      matcherWithMatchesBlock:^BOOL(id item) {
        return YES;
      }
  ...
  viewProvider = [[GREYElementProvider alloc]
      initWithRootProvider:[GREYUIWindowProvider providerWithAllWindowsWithStatusBar:NO]];
...
- (void)testNoMatchingViews {
  [gAppWindows addObject:rootWindow];
  id<GREYMatcher> switchMatcher = [GREYMatchers matcherForKindOfClass:[UISwitch class]];
  elementFinder = [[GREYElementFinder alloc] initWithMatcher:switchMatcher];
  XCTAssertEqual([elementFinder elementsMatchedInProvider:viewProvider].count, 0u,
...
- (void)testMultipleMatchingViewsFromMultipleRoots {
  ...
  id<GREYMatcher> labelMatcher = [GREYMatchers matcherForKindOfClass:[UILabel class]];
...
- (void)testElementFinderWorksWithHiddenElements {
  ...
  id<GREYMatcher> hiddenLabelMatcher =
      [GREYMatchers matcherForAccessibilityLabel:@"hiddenAccessibilityLabel"];
...
- (void)testDuplicatedViewsAsAccessibilityViewsDoNotAppearTwice {
  ...
  id<GREYMatcher> matcher = [GREYMatchers matcherForAccessibilityLabel:@"CP"];
