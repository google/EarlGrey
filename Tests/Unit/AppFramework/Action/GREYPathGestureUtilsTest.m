...
@interface GREYPathGestureUtilsTest : GREYAppBaseTest
@end
...
- (void)forEachDirectionPerformBlock:(void (^)(GREYDirection direction))block {
  GREYDirection allDirections[4] = {kGREYDirectionUp, kGREYDirectionDown, kGREYDirectionLeft,
                                    kGREYDirectionRight};
...
- (void)testDragTouchPath_cancelInertia {
  ...
  XCTAssertLessThan(fabsf((float)diff2ndLastAndLastPoint.y), fabsf((float)diffStartAndNextPoint.y));

  CGPoint path3rdLastPoint = [[path objectAtIndex:(pathLength - 3)] CGPointValue];
  CGPoint diff3rdLastAnd2ndLastPoint =
      CGPointMake(path3rdLastPoint.x - path2ndLastPoint.x, path3rdLastPoint.y - path2ndLastPoint.y);
...
- (void)DISABLED_testTouchPathWithLengthAndLeftDirection_cancelInertia {
  // TODO: The test fails. Figure out why. // NOLINT
...
- (void)testTouchPathBreaksPathsAccurately {
  ...
  OCMStub([mockVisibilityChecker rectEnclosingVisibleAreaOfElement:OCMOCK_ANY])
      .andReturn([UIScreen mainScreen].bounds);
  ...
  [self forEachDirectionPerformBlock:^(GREYDirection direction) {
    ...
    while (remainingAmount > 0) {
      ...
      CGFloat pathLength = CGVectorLength(CGVectorFromEndPoints(
          [[path firstObject] CGPointValue], [[path lastObject] CGPointValue], NO));
      pathSegmentsCount += 1;
