...
@interface NSURLConnection_GREYAdditionsTest
    : GREYAppBaseTest <NSURLConnectionDelegate, NSURLConnectionDataDelegate>
...
- (void)setUp {
  ...
  [[[[_mockGREYAppStateTracker expect] andDo:blockPending] andForwardToRealObject]
      trackState:kGREYPendingNetworkRequest
       forObject:OCMOCK_ANY];
  [[[_mockGREYAppStateTracker expect] andForwardToRealObject]
      untrackState:kGREYPendingNetworkRequest
         forObject:OCMOCK_ANY];
}
...
- (void)testConnectionClassMethodPlusDelegate {
  NSURLConnection *connection =
      [NSURLConnection connectionWithRequest:_localHostRequest delegate:self];

  [[GREYUIThreadExecutor sharedInstance] drainUntilIdle];
  [_mockGREYAppStateTracker verify];
  XCTAssertTrue(_connectionFinished, @"We shouldn't have returned until connection has finished.");
  XCTAssertEqual(connection, _capturedPendingObject, @"Unexpected object was blocked.");
}

- (void)testConnectionInitPlusDelegate {
  NSURLConnection *connection =
      [[NSURLConnection alloc] initWithRequest:_localHostRequest delegate:self];
...
- (void)testConnectionInitPlusDelegateStartLater {
  NSURLConnection *connection =
      [[NSURLConnection alloc] initWithRequest:_localHostRequest delegate:self startImmediately:NO];
  [connection start];
...
- (void)testAsyncConnectionClassMethodWithCompletionHandler {
  ...
  _connectionFinished = NO;
  [NSURLConnection
      sendAsynchronousRequest:_localHostRequest
                        queue:queue
...
- (void)testFilterConnectionChanges {
  ...
  // To make compiler happy with unused variable.
  [connection1 cancel];
  XCTAssertFalse(_connectionFinished);
...
- (void)testFilterConnectionChangesAfterConnectionStartedButBeforeFinish {
  _connectionFinished = NO;
  NSURLConnection *connection =
      [[NSURLConnection alloc] initWithRequest:_externalRequest delegate:self startImmediately:NO];
...
- (void)testCreatingNewConnectionIsTracked {
  ...
  GREYAppState state = [[GREYAppStateTracker sharedInstance] grey_lastKnownStateForObject:conn];
