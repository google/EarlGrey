...
@interface CAAnimation (CALayer_GREYAdditionsTest)
- (void)grey_trackForDurationOfAnimation;
@end

#pragma mark - CALayer Category

@interface CALayer (CALayer_GREYAdditionsTest)
...
@interface CALayer_GREYAdditionsTest : GREYAppBaseTest
@end
...
- (void)setUp {
  [super setUp];
  [GREYConfiguration.sharedConfiguration setValue:@(kMaxAnimationInterval)
...
- (void)testDrawRequestChangesPendingUIEventState {
  ...
  XCTAssertEqual(kGREYPendingDrawLayoutPass, [[GREYAppStateTracker sharedInstance] currentState],
                 @"Should change state.");
...
- (void)testAddAnimationWithLongDuration {
  ...
  XCTAssertEqual(kMaxAnimationInterval, animation.duration, @"Duration must be less than %f",
                 kMaxAnimationInterval);
}
...
- (void)testAddAnimationWithLongRepeatCount {
  ...
  XCTAssertEqualWithAccuracy(animation.repeatCount, 1, 0.1,
  ...
  XCTAssertEqualWithAccuracy(animation.repeatCount, 0, 0.0001, @"Should not change repeat count");
}
...
- (void)testAddAnimationWithLongRepeatDuration {
  ...
  XCTAssertEqualWithAccuracy(animation.repeatDuration, kMaxAnimationInterval / 2.0, 0.0001,
...
- (void)testLimitAnimationsFlag {
  ...
  [layer addAnimation:animationLongRepeatCount forKey:@"animationLongRepeatCount"];
  CAAnimation *animation = [layer animationForKey:@"animationLongRepeatCount"];
  XCTAssertEqualWithAccuracy(animation.duration, 5, 0.0001, @"Should not change duration");
  XCTAssertEqualWithAccuracy(animation.repeatCount, 10, 0.0001, @"Should not change repeat count");
}
...
- (void)testPausingAndResumingAnimationsFromParentLayer {
  ...
  XCTAssertTrue([pausedAnimationKeys containsObject:@"TestAnim1"], @"Animation was not paused!");
...
- (void)testSuccessiveAnimationPauses {
  ...
  XCTAssertEqual(pausedAnimationKeys.count, 1u,
                 @"Number of paused animations should be exactly 1!");
  XCTAssertTrue([pausedAnimationKeys containsObject:@"TestAnim2"], @"Animation was not paused!");
  GREYAppState state = [[GREYAppStateTracker sharedInstance] currentState];
