...
@interface GREYElementMatcherBlockTest : GREYAppBaseTest
...
- (void)setUp {
  ...
  self.fail1 =
      [GREYElementMatcherBlock matcherWithMatchesBlock:noMatcher descriptionBlock:fail1Description];
  self.fail2 =
      [GREYElementMatcherBlock matcherWithMatchesBlock:noMatcher descriptionBlock:fail2Description];
}
...
- (void)testDescriptionOfAllOfMatcher {
  id<GREYMatcher> matcher = [[GREYAllOf alloc] initWithMatchers:@[ self.pass1, self.pass2 ]];
  XCTAssertEqualObjects([matcher description], @"(pass1 && pass2)");
}

- (void)testDescriptionOfAnyOfMatcher {
  id<GREYMatcher> matcher = [[GREYAnyOf alloc] initWithMatchers:@[ self.pass1, self.pass2 ]];
  XCTAssertEqualObjects([matcher description], @"(pass1 || pass2)");
}

- (void)testDescriptionOfCombinationMatcher {
  id<GREYMatcher> anyOfMatcher = [[GREYAnyOf alloc] initWithMatchers:@[ self.pass1, self.pass2 ]];
  id<GREYMatcher> matcher = [[GREYAllOf alloc] initWithMatchers:@[ self.pass1, anyOfMatcher ]];
