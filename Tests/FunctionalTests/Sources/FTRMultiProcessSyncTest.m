...
@interface FTRMultiProcessSyncTest : FTRBaseIntegrationTest

@end

@implementation FTRMultiProcessSyncTest
...
- (void)testGREYExecuteSync {
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
      ...
      id<GREYMatcher> matcher = grey_allOf(grey_kindOfClass([UITextField class]),
                                           grey_accessibilityLabel(@"Type Something Here"), nil);
  ...
  [[EarlGrey selectElementWithMatcher:grey_allOf(grey_kindOfClass([UITextField class]),
                                                 ...
                                                 nil)] assertWithMatcher:grey_text(@"Hello!")];
...
- (void)testGREYExecuteAsyncOnBackgroundThread {
  ...
  id<GREYMatcher> textFieldMatcher = grey_allOf(
      grey_kindOfClass([UITextField class]), grey_accessibilityLabel(@"Type Something Here"), nil);
  // This should wait for the above async to finish.
  [[EarlGrey selectElementWithMatcher:textFieldMatcher] assertWithMatcher:grey_text(@"Hello!")];
}
