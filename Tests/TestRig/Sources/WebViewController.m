...
- (void)viewDidLoad {
  ...
  _webView.accessibilityIdentifier = @"TestWebView";
}
...
- (IBAction)userDidTapLoadLocalTestPage {
  [_webView loadRequest:[NSURLRequest requestWithURL:[WebViewController URLToTestHTMLFile]]];
  _webView.scrollView.bounces = _webViewBounceSwitch.isOn;
}

- (IBAction)userDidTapLoadHTMLUsingLoadHTMLString {
  NSString *html = [NSString stringWithContentsOfURL:[WebViewController URLToTestHTMLFile]
