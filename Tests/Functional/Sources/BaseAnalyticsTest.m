 ...
 */
NSString *const kAnalyticsHitsMisMatchedPrefix = @"Mismatched Analytics Hits";
...
@interface BaseAnalyticsTest () <GREYAnalyticsDelegate>
@end

@implementation BaseAnalyticsTest
...
+ (void)setUp {
  ...
  gTestAnalyticsDelegate = [[BaseAnalyticsTest alloc] init];
...
+ (void)classSpecificTearDown {
  ...
  if (gTotalHitsExpected != gTotalHitsReceived) {
    NSLog(@"%@, Received %d count, expected %d", kAnalyticsHitsMisMatchedPrefix,
